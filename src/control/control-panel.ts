...
export class ControlPanel {
    ...
    businessTab: TabPageApi;
    pointerPosInput: any;

    constructor(public engine: Game) {
        const ui = document.getElementById("dev-tools")
        this.pane = new Pane({
            title: 'Editor',
            expanded: true,
            container:ui
        ...
        style.innerText = ".excalibur-tweakpane-custom { padding:5px,10px,10px,10px;width: 90%; }";
        ...
        let index = 0
        this.viewTab = this.tabs.pages[index++];
        this.clockTab = this.tabs.pages[index++];
        this.timerTab = this.tabs.pages[index++];
        this.physicsTab = this.tabs.pages[index++];
        this.debugTab = this.tabs.pages[index++];
    ...
    public addListeners() {
        ...
        game.on("place.object.change", (ev) => {
            const entityId= ev["entityId"];
            console.log("place.object.change", entityId)
    ...
    /**
 updated    * `update()` is called periodically over time
     * @param devtool 
     */
    public update(devtool: ControlPanel) {
    ...
    private _buildScreenTab() {
        this.screenFolder = this.viewTab.addFolder({
            title: 'screen'
        });
    ...
    private _buildCameraTab() {
        const cameraFolder = this.viewTab.addFolder({
            title: 'camera',
        });
        ...
        cameraFolder.addInput(this.engine.currentScene.camera, "pos",{label:'position'});
    }
